# ============================================================
# Random Walk Mobility Model (Academic Version)
...
if {$argc != 3} {
    puts "Usage: ns rw.tcl <nodes> <maxspeed> <pause>"
    exit 1
}
...
# Unique filenames
set tracefile "rw_$val(nn)_$val(maxspeed)_$val(pause).tr"
set namfile   "rw_$val(nn)_$val(maxspeed)_$val(pause).nam"
...
# -----------------------------
proc random_walk {ns node maxx maxy maxspeed pause stoptime t} {
    ...
    set dx [expr $cx + (rand()*200.0) - 100.0]
    set dy [expr $cy + (rand()*200.0) - 100.0]
    ...
    set next [expr $t + $pause + 2]

    $ns at $next \
        "random_walk $ns $node $maxx $maxy $maxspeed $pause $stoptime $next"
}
